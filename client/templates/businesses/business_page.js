Template.businessPage.onCreated(function () {
  ...
  self.autorun(function () {
    if (Business.findOne({_id: Router.current().params._id})) {
      loadMap(Business.findOne({_id: Router.current().params._id}));
    }
  });
});
...
Template.businessPage.helpers({
  ...
  favorite: function () {
    return Favorite.findOne({businessId: this._id}) ? 'favorite-active' : '';
  }
});
